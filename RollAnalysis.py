...
def getPrunedSuccesses(arr :list):
...
def getExtendedActionDistribution(basePool, poolModifier, explodeChance, rote, fumbleModifier, extraRolls):
...
if doTest:
    ...
    print(rollDistribution)
    #result = getExtendedActionDistribution(basePool, poolModifier, explodeChance, rote, fumbleModifier, extraRolls)
    #print(result)
elif doAll:
    basePools = range(1, 11)
    modifiers = range(-5, 6)
    roteOptions = [False, True]
    explodeOptions = {"10a": 0.1, "9a": 0.2, "8a": 0.3}
    fumbleModifiers = [0, -3]
    extraRollOptions = [0, 2]

    start = time.time()
    f = open ("allExtendedActionPermutations.csv", "w")
    for fumbleModifier in fumbleModifiers:
        for extraRolls in extraRollOptions:
            for basePool in basePools:
                for poolModifier in modifiers:
                    if basePool + poolModifier <= 0:
                        #skip chance die rolls
                        continue
                    for rote in roteOptions:
                        for explodeChance in explodeOptions:
                            result = getExtendedActionDistribution(basePool, poolModifier, explodeOptions[explodeChance], rote, fumbleModifier, extraRolls)
                            bracketsText = ""
                            if extraRolls != 0 or fumbleModifier != 0:
                                bracketsText = ((str(extraRolls) + " extra rolls") if extraRolls != 0 else "")
                                if fumbleModifier != 0:
                                    bracketsText += (", " if len(bracketsText) > 0 else "") + str(fumbleModifier) + " on fumble"
                            permutationString = str(basePool) + (("+" + str(poolModifier)) if poolModifier != 0 else "") + " " + explodeChance + (" r" if rote else "") + ((" (" + bracketsText + ")") if len(bracketsText) > 0 else "")
                            print(permutationString + "," + result.getLinePrint())
                            f.write(permutationString + "," + result.getLinePrint() + "\n")
    f.close()
    end = time.time()
    print("execution completed: " + str(end - start))
else:
    basePool = int(input('base dicepool:'))
    poolModifier = int(input('dicepool modifier:'))
    eThres = int(input('explode threshold:'))
    explodeChance = 0.1 * (11 - eThres)
    rote = input('rote(y/n):') == 'y'
    fumbleModifier = input('fumble modifier:')
    extraRolls = input('extra rolls:')
